...
let drawPoints = () => {
    ...
    svg.selectAll('circle')
        ...
        .on('mouseover', function(event, item) {
            ...
            if(item['Doping'] != '') {
                tooltip.text(`${year}` + ' - ' + `${name}` + ' - ' + `${time}` + ' - ' + `${doping}`)
            } else {
                tooltip.text(`${year}` + ' - ' + `${name}` + ' - ' + `${time}` + ' - ' + 'No Allegations')
            }
...
req.onload = () => {
    values = JSON.parse(req.responseText); // Parses the response text as JSON and assigns it to the variable 'values'
    ...
    drawPoints(); // Calls the function 'drawPoints()'
